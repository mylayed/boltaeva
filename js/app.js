(() => {
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (targetElement.closest("[data-quantity-plus]") || targetElement.closest("[data-quantity-minus]")) {
                ...
                if (targetElement.hasAttribute("data-quantity-plus")) {
                    value++;
                    valueElement.setAttribute("value", parseInt(valueElement.value) + 1);
                    if (+valueElement.dataset.quantityMax && +valueElement.dataset.quantityMax < value) value = valueElement.dataset.quantityMax;
                } else {
                    --value;
                    if (valueElement.value > 1) valueElement.setAttribute("value", parseInt(valueElement.value) - 1);
    ...
    function initSliders() {
        ...
        if (document.querySelector(".catalog__slider")) new core(".catalog__slider", {
            ...
            effect: "fade",
            navigation: {
                prevEl: ".swiper-button-mini-prev",
                nextEl: ".swiper-button-mini-next"
            },
            on: {}
        });
        if (document.querySelector(".product__slider")) {
            const subSlider = new core(".product__preview-slider", {
                modules: [ EffectFade ],
                slidesPerView: 3,
                freeMode: true
            });
        ...
        if (document.querySelector(".recomend__slider")) new core(".recomend__slider", {
        ...
        if (document.querySelector(".checkout__slider")) new core(".checkout__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
            ...
            effect: "fade",
            navigation: {
                prevEl: ".swiper-button-check-prev",
                nextEl: ".swiper-button-check-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 2,
...
***************************************************************************** */
    ...
    function favorite(btn) {
        const favorite = document.querySelectorAll(btn);
        if (favorite) favorite.forEach((item => {
    ...
    function activeItem() {
        const curentLocation = window.location.pathname;
        const linkItem = document.querySelectorAll(".header__list-link");
        linkItem.forEach((item => {
            if (item.getAttribute("href") === curentLocation.replace(/\//, "")) item.classList.add("active");
        }));
    }
    activeItem();
    const curentInput = document.querySelector("[data-quantity-value]");
    const totalPrice = document.querySelector(".product__price span");
    const price = 54;
    const reg = /[A-Za-zA-яА-яЁё]/g;
    let currentValue;
    if (curentInput) {
        if (curentInput.value.match(/[^0-9]/g)) curentInput.value = curentInput.value.replace(/[^0-9]/g, "");
        curentInput.setAttribute("value", 1);
        currentValue = parseInt(curentInput.value);
        totalPrice.innerHTML = currentValue * price;
        console.log(currentValue);
    }
    function changePrice() {
        const quantityBtnPlus = document.querySelector(".quantity__button_plus");
        const quantityBtnMinus = document.querySelector(".quantity__button_minus");
        if (quantityBtnPlus) quantityBtnPlus.addEventListener("click", (() => {
            currentValue = parseInt(curentInput.value) + 1;
            totalPrice.innerHTML = currentValue * price;
        }));
        if (quantityBtnMinus) quantityBtnMinus.addEventListener("click", (() => {
            if (curentInput.value > 1) {
                currentValue = parseInt(curentInput.value) - 1;
                totalPrice.innerHTML = currentValue * price;
            }
        }));
        if (curentInput) curentInput.addEventListener("input", (() => {
            currentValue = parseInt(curentInput.value);
            curentInput.value = curentInput.value;
            curentInput.value = curentInput.value.replace(reg, "");
            totalPrice.innerHTML = currentValue * price;
